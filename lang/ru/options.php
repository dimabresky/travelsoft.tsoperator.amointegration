<?php

$MESS['TRAVELSOFT_AMO_CLIENT_ID'] = "Client id";
$MESS['TRAVELSOFT_AMO_CLIENT_SECRET'] = "Client secret";
$MESS['TRAVELSOFT_AMO_LONG_ACCESS_TOKEN'] = "Долгосрочный токен доступа";
$MESS['TRAVELSOFT_AMO_REDIRECT_URL'] = "Redirect url";
$MESS['TRAVELSOFT_AMO_SAVE_BTN_NAME'] = "Сохранить";
$MESS['TRAVELSOFT_AMO_RESET_BTN_NAME'] = "Сбросить";
$MESS['TRAVELSOFT_AMO_RESTORE_WARNING'] = "Все настройки будут сброшены";
$MESS['TRAVELSOFT_AMO_RESTORE_WARNING'] = "Все настройки будут сброшены";
$MESS['TRAVELSOFT_AMO_IBLOCK_ORDERCALL_ID'] = "ID инфоблока для сохранения заявок с формы заказа звонка/заказа тура";
$MESS['TRAVELSOFT_AMO_GET_TOCKEN_BTN'] = "Получить токен доступа";
$MESS['TRAVELSOFT_AMO_BASE_DOMAIN'] = "Поддомен для обращения API";
$MESS['TRAVELSOFT_AMO_NOTIFY'] = "для активации кнопки необходимо мохранить параметры";
$MESS['TRAVELSOFT_AMO_TOUR_FIELD_ID'] = "ID поля названия тура";
$MESS['TRAVELSOFT_AMO_DATE_FIELD_ID'] = "ID поля даты тура";
$MESS['TRAVELSOFT_AMO_DATE_END_FIELD_ID'] = "ID поля окончания даты тура";
$MESS['TRAVELSOFT_AMO_DESC_FIELD_ID'] = "ID поля описания сделки";
$MESS['TRAVELSOFT_AMO_TOUR_LINK_FIELD_ID'] = "ID поля ссылки на страницу тура";
$MESS['TRAVELSOFT_AMO_ADULTS_FIELD_ID'] = "ID поля кол-ва взрослых";
$MESS['TRAVELSOFT_AMO_TOTAL_PEOPLE_FIELD_ID'] = "ID поля общего количества человек";
$MESS['TRAVELSOFT_AMO_BUS_SEAT_FIELD_ID'] = "ID поля места в автобусе";
$MESS['TRAVELSOFT_AMO_ACCOMMODATION_TYPE_FIELD_ID'] = "ID поля тип размещения";
$MESS['TRAVELSOFT_AMO_ORDER_NUMBER_FIELD_ID'] = "ID поля номер заявки";
$MESS['TRAVELSOFT_AMO_CHILDREN_FIELD_ID'] = "ID поля кол-ва детей";
$MESS['TRAVELSOFT_AMO_PHONE_FIELD_ID'] = "ID поля телефона";
$MESS['TRAVELSOFT_AMO_CID_FIELD_ID'] = "ID поля CID";
$MESS['TRAVELSOFT_AMO_STATUS_ID'] = "ID Статуса сделки по-умолчанию";
$MESS['TRAVELSOFT_AMO_PIPELINE_ID'] = "ID Воронки сделки по-умолчанию";
$MESS['TRAVELSOFT_AMO_CALLBACK_FORM_IBLOCK_ID'] = "ID инфоблока для сохранения заявок с формы заказа звонка/заказа тура";

